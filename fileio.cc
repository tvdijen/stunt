...
bf_file_readlines(Var arglist, Byte next, void *vdata, Objid progr)
{
  package r;
  Var fhandle = arglist.v.list[1];
  int32_t begin = arglist.v.list[2].v.num;
  int32_t end   = arglist.v.list[3].v.num;
...
bf_file_read(Var arglist, Byte next, void *vdata, Objid progr)
{
  ...
  file_mode mode;
  file_type type;
  int32_t record_length = arglist.v.list[2].v.num;
  int32_t read_length;
...
bf_file_seek(Var arglist, Byte next, void *vdata, Objid progr)
{
  package r;
  Var fhandle = arglist.v.list[1];
  int32_t seek_to = arglist.v.list[2].v.num;
